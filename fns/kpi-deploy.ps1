...
try {
Invoke-command –computername $DeployHost -credential $Cred –scriptblock { D:\Builds\trc-graceful-teardown-local.ps1 }
}
Catch { BREAK }
...
Invoke-command –computername $DeployHost -credential $Cred –scriptblock { Copy-Item -Path K:\Builds\Build$Stamp -Destination K:\AIS3 –Recurse -Force }
}
Catch { BREAK }

try {
Invoke-command –computername $DeployHost -credential $Cred -argumentlist $Stamp –scriptblock { 
