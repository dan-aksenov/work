...
psql $db_dest -c "select datname, pg_catalog.pg_size_pretty(pg_catalog.pg_database_size(d.datname)) AS Size, (pg_stat_file('base/'||oid||'/PG_VERSION')).modification AS datcreated from pg_database where datname = '${dbname}_tmp'"
...
psql $db_dest -c "select datname, pg_catalog.pg_size_pretty(pg_catalog.pg_database_size(d.datname)) AS Size, (pg_stat_file('base/'||oid||'/PG_VERSION')).modification AS datcreated from pg_database where datname = '${dbname}'"
